[**@vercel/oidc-aws-credentials-provider**](../README.md)

---

# Type Alias: AwsCredentialsProviderInit

> **AwsCredentialsProviderInit** = [`Omit`](https://www.typescriptlang.org/docs/handbook/utility-types.html#omittype-keys)\<`FromWebTokenInit`, `"webIdentityToken"`\> & [`VercelOidcExchangeInit`](VercelOidcExchangeInit.md)

Defined in: [aws-credentials-provider.ts:59](https://github.com/gwozai/brunch-ver/blob/main/packages/oidc-aws-credentials-provider/src/aws-credentials-provider.ts#L59)
